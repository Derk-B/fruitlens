...
-- Generate initial model with random values
...
-- Apply activation function on feedforward result
...
deltas inputs labels layers = let
  (avs@(av:_), zv:zvs) = revaz inputs layers
  delta0 = zipWith (*) (zipWith dCost av labels) (relu' <$> zv)
...
learn inputs labels layers = let (avs, dvs) = deltas inputs labels layers
  in zip (zipWith descend (fst <$> layers) dvs) $
    zipWith3 (\wvs av dv -> zipWith (\wv d -> descend wv ((d*) <$> av)) wvs dv)
      (snd <$> layers) avs dvs

-- Helper functions for getting values from datasets
...
-- Define steps to get intermediate training statistics
  let
